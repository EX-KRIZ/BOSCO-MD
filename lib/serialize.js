...
async function serialize(msg, conn, store) {
  ...
  let vM = async(message, M ) 
message.quoted.fakeObj = M.fromObject({
                key: {
...
conn.forwardMessage = (jid, forceForward = true, options = {}) => conn.copyNForward(jid, vM, forceForward,{contextInfo: {isForwarded: false}}, options)
